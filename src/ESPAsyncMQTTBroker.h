...
enum DebugLevel
{
    ...
    DEBUG_INFO = 2,  ///< Fehler und Informationen werden angezeigt
    DEBUG_DEBUG = 3  ///< Alle Details werden angezeigt (inklusive Debug-Informationen)
};
...
struct RetainedMessage
{
    ...
    RetainedMessage(const String& t, const uint8_t* p, size_t len, uint8_t q) 
        ...
        if (len > 0 && p != nullptr) {
            payload = std::make_unique<uint8_t[]>(len);
