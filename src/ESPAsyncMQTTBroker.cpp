...
void ESPAsyncMQTTBroker::handlePublish(MQTTClient* client, uint8_t* data, uint32_t length, uint8_t header) {
    ...
    if (2 + topicLength > length) return;
    ...
    uint16_t packetId = 0;
    if (qos > 0) {
        if (payloadOffset + 2 > length) return;
...
void ESPAsyncMQTTBroker::handleSubscribe(MQTTClient* client, uint8_t* data, uint32_t length) {
    ...
    while (index < length) {
        ...
        index += 2;
        if (index + topicLength > length) break;
        char topicBuffer[256];
...
bool ESPAsyncMQTTBroker::authenticateClient(const String& username, const String& password) {
    if (debugLevel >= DEBUG_DEBUG) {
        Serial.println(String("🔐 Authentifizierungsversuch:"));
        Serial.println(String("   • Empfangener Username: '") + username + "'");
        Serial.println(String("   • Empfangenes Passwort: '") + (password.isEmpty() ? "[leer]" : "********") + "'");
    ...
    if (username == "") {
        if (debugLevel >= DEBUG_INFO) {
            Serial.println("✅ Anonyme Verbindung erlaubt (auch mit konfiguriertem Benutzernamen)");
        }
        return true;
    }
    if (username != brokerConfig.username) {
        if (debugLevel >= DEBUG_ERROR) {
            Serial.println("❌ Ungültiger Benutzername");
        }
        return false;
    }
    if (brokerConfig.password == "") {
        if (debugLevel >= DEBUG_INFO) {
            Serial.println("✅ Authentifizierung mit Benutzername erfolgreich (kein Passwort erforderlich)");
        }
        return true;
    }
    ...
    if (debugLevel >= DEBUG_INFO) {
        Serial.println("✅ Authentifizierung mit Benutzername und Passwort erfolgreich");
    }
    return true;
}
