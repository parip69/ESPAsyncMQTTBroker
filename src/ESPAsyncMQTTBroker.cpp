...
{
    logMessage(DEBUG_DEBUG, "MQTT CONNECT packet received:");

    if (length < 10)
    {
        logMessage(DEBUG_ERROR, "Packet too short!");
        return;
    }
    ...
    {
        logMessage(DEBUG_ERROR, "Packet too short for protocol name!");
        return;
    }
    ...
    if (offset >= length)
    {
        logMessage(DEBUG_ERROR, "Packet too short for protocol level!");
        return;
    }
    ...
    if (offset >= length)
    {
        logMessage(DEBUG_ERROR, "Packet too short for CONNECT flags!");
        return;
    }
    ...
    if (offset + 2 > length)
    {
        logMessage(DEBUG_ERROR, "Packet too short for Keep-Alive!");
        return;
    }
    ...
    if (offset + 2 > length)
    {
        logMessage(DEBUG_ERROR, "Packet too short for Client-ID!");
        return;
    }
    ...
    {
        logMessage(DEBUG_ERROR, "Packet too short for full Client-ID!");
        return;
    }
    ...
    {
        logMessage(DEBUG_INFO, "Reusing persistent session for client: %s", clientId.c_str());
    ...
    {
        logMessage(DEBUG_INFO, "Authentication successful - connection accepted");
    }
