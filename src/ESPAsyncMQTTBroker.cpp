...
ESPAsyncMQTTBroker::ESPAsyncMQTTBroker(uint16_t port)
    : port(port)
...

{
...

  {
    ...
    broker->checkTimeoutsFlag =
        true; // Nur Flag setzen, Verarbeitung in loop() (BP1-01)
  };
...
void ESPAsyncMQTTBroker::checkTimeouts() {
  ...
  for (auto it = clients.begin(); it != clients.end();) {
    ...
    if (mqttClient->connected && mqttClient->keepAlive > 0 &&
      ...
      logMessage(DEBUG_INFO, "Client ⏰ inactive, disconnecting: %s",
      ...
      for (auto msgIt = mqttClient->outgoingMessages.begin();
           msgIt != mqttClient->outgoingMessages.end();) {
        auto &outMsg = msgIt->second;
        if (now - outMsg.sentTime > retryTimeout) {
          if (outMsg.retryCount >= maxRetries) {
            logMessage(DEBUG_ERROR,
                       "QoS %d message for client '%s' (packet ID %u) timed "
                       "out after %d retries. Discarding.",
                       outMsg.qos, mqttClient->clientId.c_str(),
                       outMsg.packetId, maxRetries);
            ...
            logMessage(DEBUG_INFO,
                       ...
                       "out. Retrying (%d/%d)...",
                       outMsg.qos, mqttClient->clientId.c_str(),
                       outMsg.packetId, outMsg.retryCount + 1, maxRetries);
            outMsg.retryCount++;
            outMsg.sentTime = now;
            if (outMsg.state == OutgoingQoSState::AwaitingPuback ||
                outMsg.state == OutgoingQoSState::AwaitingPubrec) {
              // BP2-01: Resend PUBLISH with DUP flag — Variable-Length-Encoding
              // für Remaining-Length
              size_t topicLen = outMsg.topic.length();
              size_t packet_id_len = 2;
              size_t remainingLength =
                  2 + topicLen + packet_id_len + outMsg.payloadLen;

              // Header-Länge berechnen (1 Byte Fixheader +
              ...
              *ptr++ = (MQTT_PUBLISH << 4) | (outMsg.qos << 1) |
                       (outMsg.retain ? 1 : 0) | 0x08; // Set DUP flag
              ...
              *ptr++ = outMsg.packetId & 0xFF;
              ...
              // Resend PUBREL
              uint8_t pubrel[] = {0x62, 0x02, (uint8_t)(outMsg.packetId >> 8),
                                  (uint8_t)(outMsg.packetId & 0xFF)};
...

{
  ...
  // Der debugLevel wird via BROKER_DEBUG_LEVEL Build-Flag in der platformio.ini
  // gesetzt und sollte NICHT durch setConfig() überschrieben werden. Der
  // Benutzer kann alternativ setDebugLevel() direkt aufrufen, wenn gewünscht.

  logMessage(DEBUG_INFO, "🔧 MQTT-Broker Configuration:");

  logMessage(DEBUG_INFO, "   Username: %s",
             (brokerConfig.username.isEmpty() ? "[empty]"
                                              : brokerConfig.username.c_str()));

  logMessage(DEBUG_INFO, "   Password: %s",
             (brokerConfig.password.isEmpty() ? "[empty]" : "[set]"));

  logMessage(DEBUG_INFO, "   Auth required: %s",
             (brokerConfig.username != "" ? "Yes" : "No"));
}
...

{
...

      {
        ...
        if (it != broker->clients.end()) {
          ...
          if (len > MQTT_MAX_PACKET_SIZE) {
            ...
            broker->logMessage(DEBUG_ERROR,
                               "Packet size exceeds limit: %u > %u",
...

      {
        ...
        if (it != broker->clients.end()) {
          ...
          if (target->hasWill && !target->gracefulDisconnect) {
            ...
            broker->logMessage(DEBUG_INFO,
                               "Unclean disconnect from client %s. Publishing "
                               "LWT: Topic='%s', QoS=%d, Retain=%s",

                               target->clientId.c_str(),
                               target->willTopic.c_str(), target->willQos,
                               target->willRetain ? "Yes" : "No");

            broker->publish(target->willTopic.c_str(),
                            target->willPayload.get(), target->willPayloadLen,
                            target->willRetain, target->willQos, "");

            target->hasWill = false;

          } else if (target->hasWill && target->gracefulDisconnect) {

            broker->logMessage(DEBUG_DEBUG,
                               "LWT for client %s not sent (clean disconnect "
                               "already handled).",
                               target->clientId.c_str());
          }
          ...
          String disconnectedClientId =
              target->clientId; // Vor std::move sichern

          // QoS2-Eingangs-State liegt jetzt im MQTTClient.
          // Bei cleanSession wird er mit dem Client zerstört.
          // Bei persistenter Session bleibt er zusammen mit dem Client
          // erhalten.
          ...
          if (!target->cleanSession) {
            ...
            broker->logMessage(
                DEBUG_INFO,
                "Client %s disconnected (graceful: %s), session will be kept.",

                target->clientId.c_str(),
                target->gracefulDisconnect ? "Yes" : "No");
            ...
            broker->logMessage(DEBUG_INFO,
                               "Client %s disconnected (graceful: %s), Clean "
                               "Session, removing client.",
...

      {
        ...
        if (it != broker->clients.end()) {
          ...
          if (broker->errorCallback && mqttClient) {
            ...
            broker->logMessage(DEBUG_ERROR, "Client %s Error: %d",
  ...
  logMessage(DEBUG_DEBUG, "New MQTT connection accepted (IP: %s)",
             client->remoteIP().toString().c_str());
}

void ESPAsyncMQTTBroker::processPacket(MQTTClient *client, uint8_t *data,
                                       size_t len)
...
void ESPAsyncMQTTBroker::handleConnect(MQTTClient *client, uint8_t *data,
                                       size_t length) {
...

  {
    ...
    logMessage(DEBUG_ERROR,
               "REJECT: Empty ClientID not allowed if cleanSession=false");
...

  {
    ...
    logMessage(DEBUG_INFO,
               "♻️ Persistente Session wiederhergestellt für Client: %s",
               clientId.c_str());
  ...
  logMessage(DEBUG_INFO, "[BROKER] CONNECT cid=%s kaSec=%d", clientId.c_str(),
             keepAlive);
...

  {
...

    {
      ...
      logMessage(DEBUG_WARNING, "Will-Payload wird gekürzt auf %u (von %u)",
                 MQTT_MAX_PAYLOAD_SIZE, willPayloadActualLen);
  ...
  logMessage(DEBUG_DEBUG,
             ...
             "CONNECT: proto='%s'(lvl=%u), flags=0x%02X [clean=%d, will=%d, "
             "usr=%d, pwd=%d], keepAlive=%u, clientId='%s'",

             protocolName, (unsigned)protocolLevel, (unsigned)connectFlags,

             (int)cleanSession, (int)willFlag, (int)usernameFlag,
             (int)passwordFlag,
...

  {
...

    {
      ...
      logMessage(DEBUG_ERROR,
                 "REJECT: Mode=USER_ONLY -> username flag missing");
...

  {
...

    {
      ...
      logMessage(
          DEBUG_ERROR,
          "REJECT: Mode=USER_PASS -> required flag(s) missing (usr=%d, pwd=%d)",
  ...
  if (debugLevel >= DEBUG_INFO) {
    String cfgUserStr =
        brokerConfig.username.isEmpty() ? "<empty>" : brokerConfig.username;
    String cfgPassStr = brokerConfig.password.isEmpty() ? "<empty>" : "<set>";
    String policyMode =
        (!cfgUserSet) ? "ANON" : (!cfgPassSet ? "USER" : "USER+PASS");
    String tryStr =
        String(usernameFlag ? "U" : "-") + String(passwordFlag ? "P" : "-");
    String userInStr =
        usernameFlag ? (username.isEmpty() ? "<none>" : username) : "<none>";
    String passInStr =
        passwordFlag ? (password.isEmpty() ? "<none>" : "<present>") : "<none>";
    ...
    // BP3-01: Zusammenfassung nur bei DEBUG_DEBUG (Auth-Frame oben enthält
    // bereits alle Infos)
    logMessage(DEBUG_DEBUG, "--- MQTT Client Connect Info ---");
    logMessage(DEBUG_DEBUG, "ClientID      : %s", client->clientId.c_str());
    logMessage(DEBUG_DEBUG, "Username      : '%s' (len=%u)", username.c_str(),
               (unsigned)username.length());
    logMessage(DEBUG_DEBUG, "Password      : %s (len=%u)",
               password.isEmpty() ? "<empty>" : "<set>",
               (unsigned)password.length());
    logMessage(DEBUG_DEBUG, "Flags(usr/pwd): %d / %d", (int)usernameFlag,
               (int)passwordFlag);
    logMessage(DEBUG_DEBUG, "CleanSession  : %s",
               cleanSession ? "true" : "false");
    logMessage(DEBUG_DEBUG, "KeepAlive     : %u", (unsigned)keepAlive);
    logMessage(DEBUG_DEBUG, "ProtoVersion  : %u",
               (unsigned)client->protocolVersion);
    logMessage(DEBUG_DEBUG, "--------------------------------");
  }
...

  {
    ...
    connectedClientsInfo[client->clientId] =
        ipStr; // Immer aktualisieren, nicht nur bei Callback (BP2-06)

    if (clientConnectCallback) {
      clientConnectCallback(client->clientId, ipStr, username,
                            password.length());
    }
  }
...
void ESPAsyncMQTTBroker::handlePublish(MQTTClient *client, uint8_t *data,
...

{
...

  {
    ...
    logMessage(DEBUG_ERROR, "Topic too long: %u > %u", topicLength,
               MQTT_MAX_TOPIC_SIZE);
...

  {
    ...
    logMessage(DEBUG_ERROR,
               "Invalid Topic Name '%s' from client '%s'. Closing connection.",
...

  {
...

    {
      ...
      uint8_t puback[] = {0x40, 0x02, (uint8_t)(packetId >> 8),
                          (uint8_t)packetId};
...

    {
...

      {
        ...
        logMessage(DEBUG_WARNING,
                   "QoS 2 Payload will be truncated to %u (from %u)",
                   MQTT_MAX_PAYLOAD_SIZE, payloadLength);

        payloadLength = MQTT_MAX_PAYLOAD_SIZE;
      }

      IncomingQoS2Message qos2Msg(topic, data + payloadOffset, payloadLength,
                                  retained, client->clientId);
      ...
      logMessage(
          DEBUG_INFO,
          "QoS 2 Publish received - Topic='%s', PacketID=%u. Sending PUBREC.",
          topic.c_str(), packetId);

      uint8_t pubrec[] = {(MQTT_PUBREC << 4), 0x02, (uint8_t)(packetId >> 8),
                          (uint8_t)packetId};
...

  {
...

    {
...

      {
        ...
        logMessage(DEBUG_WARNING, "Payload will be truncated to %u (from %u)",
                   MQTT_MAX_PAYLOAD_SIZE, payloadLength);

        payloadLength = MQTT_MAX_PAYLOAD_SIZE;
      }

      // Payload direkt als String ohne 769-Byte Stack-Buffer konstruieren
      // (BP1-03)
      String originalPayload;
      originalPayload.concat((const char *)(data + payloadOffset),
                             payloadLength);

      // NEU: Payload mit Quelle-Präfix versehen
      String newPayload =
          "source:[" + client->clientId + "];" + originalPayload;
      if (newPayload.length() > MQTT_MAX_PAYLOAD_SIZE) {
        logMessage(DEBUG_WARNING, "Payload mit Source-Präfix überschreitet die "
                                  "maximale Größe und wird gekürzt.");
        newPayload = newPayload.substring(0, MQTT_MAX_PAYLOAD_SIZE);
      }

      logMessage(
          DEBUG_INFO,
          "🔔 Weiterleiten (QoS %d, von %s) - Topic='%s', NeuerPayload='%s'",
          qos, client->clientId.c_str(), topic.c_str(), newPayload.c_str());

      publish(topic.c_str(), newPayload.c_str(), retained, qos,
              client->clientId);
...

    {
      ...
      logMessage(DEBUG_INFO, "Publish (QoS %d, empty Retained) - Topic='%s'",
                 qos, topic.c_str());
...
void ESPAsyncMQTTBroker::handleSubscribe(MQTTClient *client, uint8_t *data,
                                         size_t length)

{
...

  {
...

    {
      ...
      logMessage(DEBUG_ERROR, "Subscribe topic too long: %u > %u", topicLength,
                 MQTT_MAX_TOPIC_SIZE);
...

    {
      ...
      // BP2-04: Doppelte Subscriptions verhindern — MQTT-Spec erlaubt Update,
      // nicht doppeln
      bool found = false;
      for (auto &existing : client->subscriptions) {
        if (existing.filter == topic) {
          ...
          found = true;
          logMessage(DEBUG_DEBUG,
                     "Subscription for client '%s' to topic '%s' updated "
                     "(noLocal %s).",
      ...
      logMessage(DEBUG_INFO,
                 "Subscription for client '%s' to topic filter '%s' added (QoS "
                 "%d, noLocal %s).",
                 client->clientId.c_str(), topic.c_str(), requestedQoS,
                 noLocal ? "Yes" : "No");
...

    {
      ...
      logMessage(
          DEBUG_WARNING,
          "Subscription for client '%s' to invalid topic filter '%s' rejected.",
          client->clientId.c_str(), topic.c_str());
...
void ESPAsyncMQTTBroker::handleUnsubscribe(MQTTClient *client, uint8_t *data,
                                           size_t length)

{
  ...
  uint8_t unsuback[4] = {0xB0, 0x02, (uint8_t)(packetId >> 8),
                         (uint8_t)packetId};
...

  {
...

    {
      ...
      logMessage(DEBUG_ERROR, "Unsubscribe topic too long: %u > %u",
...
void ESPAsyncMQTTBroker::handlePingReq(MQTTClient *client) {
  ...
  if (!client->kaSeen) {
    client->kaSeen = true;
    logMessage(DEBUG_INFO, "[BROKER] KA REGISTERED cid=%s",
...

{
  ...
  logMessage(DEBUG_INFO,
             "Clean disconnect from client %s (DISCONNECT packet received).",
             client->clientId.c_str());
...

  {
    ...
    logMessage(DEBUG_DEBUG,
               "LWT for client %s is discarded (clean disconnect).",
               client->clientId.c_str());
...
void ESPAsyncMQTTBroker::handlePuback(MQTTClient *client, uint8_t *data,
                                      size_t len)

{
...

  {
...

    {
      ...
      logMessage(DEBUG_DEBUG,
                 "PUBACK from subscriber '%s' for packet ID %u received.",
                 client->clientId.c_str(), packetId);
...

    {
      ...
      logMessage(DEBUG_WARNING,
                 "Received PUBACK for QoS 2 message from '%s' (packet ID %u). "
                 "This is unexpected.",
                 client->clientId.c_str(), packetId);
    }
  }
...

  {
    ...
    logMessage(DEBUG_DEBUG,
               "Spurious PUBACK from '%s' for packet ID %u received.",
               client->clientId.c_str(), packetId);
  }
}

void ESPAsyncMQTTBroker::handlePubRec(MQTTClient *client, uint8_t *data,
                                      size_t len)

{
  ...
  if (it != client->outgoingMessages.end() &&
      it->second.state == OutgoingQoSState::AwaitingPubrec)

  {

    logMessage(DEBUG_DEBUG,
               "PUBREC from subscriber '%s' for packet ID %u received.",
               client->clientId.c_str(), packetId);
    ...
    uint8_t pubrel[] = {0x62, 0x02, (uint8_t)(packetId >> 8),
                        (uint8_t)(packetId & 0xFF)};

    client->client->write((const char *)pubrel, sizeof(pubrel));

    logMessage(DEBUG_DEBUG,
               "Sending PUBREL to subscriber '%s' for packet ID %u.",
               client->clientId.c_str(), packetId);
  ...
  logMessage(DEBUG_DEBUG, "PUBREC for publisher packet ID %u processed",
             packetId);
}

void ESPAsyncMQTTBroker::handlePubRel(MQTTClient *client, uint8_t *data,
                                      size_t len)

{
...

  {
    ...
    logMessage(DEBUG_WARNING, "PUBREL for unknown packet ID %u received.",
               packetId);
  }

  uint8_t pubcomp[] = {(MQTT_PUBCOMP << 4), 0x02, (uint8_t)(packetId >> 8),
                       (uint8_t)packetId};
...
void ESPAsyncMQTTBroker::handlePubComp(MQTTClient *client, uint8_t *data,
                                       size_t len)

{
...

  {
    ...
    logMessage(DEBUG_DEBUG,
               "PUBCOMP from subscriber '%s' for packet ID %u received. QoS 2 "
               "flow complete.",
               client->clientId.c_str(), packetId);
  ...
  logMessage(DEBUG_DEBUG, "PUBCOMP for publisher packet ID %u received",
             packetId);
}

bool ESPAsyncMQTTBroker::topicMatches(const Subscription &subscription,
                                      const String &topic)
...

{
...

  {
...

    {
      ...
      logMessage(DEBUG_ERROR,
                 "Error: Invalid unique_ptr in retainedMessages Map found.");
...

    {
...

      {
...

        {
          ...
          logMessage(DEBUG_ERROR, "Retained Topic too long: %u > %u",
...

        {
          ...
          logMessage(
              DEBUG_WARNING,
              "Retained Payload for Topic '%s' will be truncated: %u > %u",
...

        {
          ...
          logMessage(DEBUG_ERROR,
                     "Retained Message (Topic: %s) exceeds "
                     "MQTT_MAX_PACKET_SIZE: %u > %u.",
                     msg->topic.c_str(), (unsigned)totalPacketLength,
        ...
        *ptr++ = topicLength & 0xFF;
        ...
        logMessage(
            DEBUG_DEBUG,
            "Retained Message sent: Topic='%s', Payload-length=%u, QoS=%d",
...
bool ESPAsyncMQTTBroker::authenticateClient(const String &username,
                                            const String &password) {
  // ANON: keine Auth konfiguriert -> alles akzeptieren
  if (authAnonMode) {
    if (brokerConfig.log) {
      logMessage(DEBUG_INFO, "[AUTH] Mode=ANON: Broker akzeptiert alle "
                             "anonymen Clients. -> Accept");
    }
    return true;
  }
  ...
  if (!userOk) {
    if (brokerConfig.log) {
      logMessage(DEBUG_ERROR,
                 "[AUTH] Username '%s' nicht in erlaubter Liste -> Reject",
                 u.c_str());
    }
    return false;
  }
  ...
  if (p.isEmpty()) {
    if (brokerConfig.log) {
      logMessage(DEBUG_ERROR,
                 "[AUTH] Mode=USER_PASS: Passwort fehlt/leer -> Reject");
    }
    return false;
  }
  ...
  if (p.length() != brokerConfig.password.length()) {
    if (brokerConfig.log) {
      logMessage(DEBUG_ERROR,
                 "[AUTH] Mode=USER_PASS: Passwort-Länge passt nicht -> Reject");
    }
    return false;
  }
  ...
  if (!passOk) {
    if (brokerConfig.log) {
      logMessage(DEBUG_ERROR,
                 "[AUTH] Mode=USER_PASS: Passwort falsch -> Reject");
    }
    return false;
  }
...

{
...

  {
    ...
    logMessage(DEBUG_WARNING, "Portänderung auf %u abgelehnt – Server läuft",
               (unsigned)newPort);
  ...
  logMessage(DEBUG_INFO,
             "Broker-Port gesetzt auf %u (wirksam bei nächstem begin())",
             (unsigned)newPort);
...

{
...

  {
    ...
    logMessage(DEBUG_WARNING,
               "Invalid publish topic: Topic '%s' exceeds max length of %d.",
               topic.c_str(), MQTT_MAX_TOPIC_SIZE);
...

  {
    ...
    logMessage(
        DEBUG_WARNING,
        "Invalid publish topic: Topic '%s' contains multi-level wildcard '#'.",
        topic.c_str());
...

  {
    ...
    logMessage(
        DEBUG_WARNING,
        "Invalid publish topic: Topic '%s' contains single-level wildcard '+'.",
        topic.c_str());
...
bool ESPAsyncMQTTBroker::publish(const char *topic, const char *payload,
...

{
...

  {
    ...
    logMessage(DEBUG_DEBUG, "Null pointer as payload for C-String Publish, "
...
bool ESPAsyncMQTTBroker::publish(const char *topic, uint8_t qos, bool retained,
                                 const char *payload)
...
bool ESPAsyncMQTTBroker::publish(const char *topic, const uint8_t *payload,
                                 size_t payloadLen, bool retained, uint8_t qos,
                                 const String &excludeClientId)

{
...

  {
    ...
    logMessage(DEBUG_ERROR,
               "Null pointer as payload with payloadLen > 0 for Publish");
...

  {
    ...
    logMessage(DEBUG_ERROR, "Topic too long: %u > %u", (unsigned)topicLen,
               MQTT_MAX_TOPIC_SIZE);
...

  {
    ...
    logMessage(DEBUG_WARNING, "Payload will be truncated: %u > %u",
  ...
  logMessage(DEBUG_INFO,
             "📤 Broker is publishing on topic '%s' (Length: %u, QoS: %d, "
             "Retained: %s)",
             topic, (unsigned)payloadLen, qos, retained ? "Yes" : "No");
...

  {
...

    {
      ...
      auto msg =
          std::make_unique<RetainedMessage>(topicStr, payload, payloadLen, qos);
...

  {
    ...
    if (!excludeClientId.isEmpty() && c->clientId == excludeClientId) {
      logMessage(DEBUG_DEBUG,
                 "  - Client %s (Original Publisher) will be skipped",
                 c->clientId.c_str());
      continue;
    }
...

    {
...

      {
        ...
        uint8_t final_qos = qos; // We could downgrade QoS here based on
                                 // subscription, but for now use original.
        ...
        { // Max for 3 bytes
          ...
          logMessage(DEBUG_ERROR, "Message too large to encode. Topic: %s",
                     topicStr.c_str());
        ...
        *ptr++ = topicLen & 0xFF;
...

        {
          ...
          *ptr++ = packetId & 0xFF;
...

          {
            ...
            outMsg->payload =
                std::unique_ptr<uint8_t[]>(new uint8_t[payloadLen]);
          ...
          outMsg->state = (final_qos == 1) ? OutgoingQoSState::AwaitingPuback
                                           : OutgoingQoSState::AwaitingPubrec;

          c->outgoingMessages[packetId] = std::move(*outMsg);

          logMessage(
              DEBUG_DEBUG,
              "Storing outgoing QoS %d message for client '%s' (packet ID %u)",
              final_qos, c->clientId.c_str(), packetId);
        }
        ...
        bool writeSuccess =
            c->client->write((const char *)packet.get(), packetSize);
        ...
        logMessage(DEBUG_DEBUG, "  - Sent PUBLISH to %s (QoS %d), Success: %s",
                   c->clientId.c_str(), final_qos, writeSuccess ? "Yes" : "No");

        break; // Message sent to this client for this topic, move to next
               // client
      }
    }
  }

  logMessage(DEBUG_INFO, "📊 Message sent to %d of %d connected clients",
...

{
...

  {
    ...
    logMessage(DEBUG_WARNING,
               "Invalid topic filter: Filter exceeds 65535 bytes.");
...

  {
    ...
    logMessage(DEBUG_WARNING,
               "Invalid topic filter: Could not split levels for non-empty "
               "filter '%s'.",
               filter.c_str());
...

  {
...

    {
...

      {
        ...
        logMessage(DEBUG_WARNING,
                   "Invalid topic filter: '#' cannot be part of a level "
                   "(Level: '%s', Filter: '%s').",
                   level.c_str(), filter.c_str());
...

      {
        ...
        logMessage(
            DEBUG_WARNING,
            "Invalid topic filter: '#' must be the last level (Filter: '%s').",
            filter.c_str());
